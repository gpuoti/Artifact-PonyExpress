...
def store(instructions=None, metainfo=None, json=None, file =None, db_connection_info=MongoConnectionInfo()):
    if(json):
        store_json(json, db_connection_info=db_connection_info)
    elif(instructions and metainfo):
        store_low_level(instructions, metainfo, db_connection_info=db_connection_info )
    elif(file):
        store_json_file(file, db_connection_info=db_connection_info)
   
def store_low_level(instructions, metainfo, db_connection_info=MongoConnectionInfo()):
    ...
    connected_portfolio.store(package_content.getvalue(), metainfo)
    
    if not silent:
        print ("package was " + str(int(len(package_content.getvalue()) / 1024)) + "kB") 

def store_json_file(path, db_connection_info=MongoConnectionInfo()):
    f = open(path, "r")
    store_json(f.read(), db_connection_info=db_connection_info)
    f.close()
    
    
def store_json(json_text, db_connection_info=MongoConnectionInfo()):
    ...
    store(instructions, jobject, db_connection_info=db_connection_info)
    
def bring_one(instructions, meta_request, db_connection_info=MongoConnectionInfo()):
...
def bring(instructions, meta_requests, db_connection_info=MongoConnectionInfo()):
    """bring artifacts defined by each of meta_requests then apply unbox instructions to each."""
    created_files = []
    connected_portfolio = Portfolio(db_connection_info)
    ...
    if isinstance(meta_requests, list):
        for r in meta_requests:
            print("REQUIRE> "+ str(r))
            created_files += bring_one(instructions, r, db_connection_info=db_connection_info)
    else:
        print("REQUIRE FAILS> "+ str(meta_requests))
    
    return [s.replace('\\','/') for s in created_files], gr

def bring_json(json_text, db_connection_info=MongoConnectionInfo()):
    jobject = json.loads(json_text)
    ...
    files, gr = bring(instructions, jobject["DEPENDENCIES"], db_connection_info=db_connection_info)
    print ("bring log" )
    for f in files:
        print (str(f))
